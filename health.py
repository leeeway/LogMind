...
async def check_elasticsearch() -> ComponentHealth:
    """Probe Elasticsearch connectivity with ping (no cluster privileges needed)."""
    import asyncio
    try:
        ...
        t0 = time.monotonic()
        es = get_es_client()
        alive = await asyncio.wait_for(es.ping(), timeout=5.0)
        latency = (time.monotonic() - t0) * 1000
        ...
        logger.warning("health_es_timeout", timeout_s=5)
        return ComponentHealth(status="degraded", error="ES ping timed out (5s)")
    except Exception as e:
        logger.error("health_es_failed", error=str(e))
        return ComponentHealth(status="degraded", error=str(e)[:200])
