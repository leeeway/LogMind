...
export default function HttpAccessSites() {
  ...
  const load = async () => {
    setLoading(true);
    try { const { data } = await httpAccessApi.listSites({ ...(search ? { search } : {}), ...(mode ? { monitoring_mode: mode } : {}) }); setItems(data.items); }
  ...
  return <Card title="HTTP 访问站点治理" extra={<Button onClick={load}>刷新</Button>}>
